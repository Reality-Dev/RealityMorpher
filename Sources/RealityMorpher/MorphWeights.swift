...
/// Describes the weights of up to 8 morph targets.
///
...
@dynamicMemberLookup public struct MorphWeights: Animatable, ExpressibleByArrayLiteral {
    ...
    private(set) public var values: SIMD8<Float>
    
    public var animatableData: SIMD8<Float> {
        get { values }
        set { values = newValue }
    }
    ...
    public init(_ weights: [Float]) {
        ...
        self.init(values: SIMD8(elements[0..<MorphEnvironment.maxTargetCount]))
    }
    ...
    public init(values: SIMD8<Float>) {
        self.values = values
    }
    ...
    public subscript(dynamicMember keyPath: KeyPath<SIMD8<Float>, Float>) -> Float {
